...
INSERT INTO `approvals` (`id`, `submission_id`, `user_id`, `department_id`, `status`, `note`, `created_at`, `updated_at`) VALUES
(1,	1,	12,	1,	'rejected',	'Kurang Mantab',	'2022-09-25 12:58:15',	'2022-09-25 12:58:15'),
(2,	2,	12,	1,	'rejected',	NULL,	'2022-09-26 13:41:18',	'2022-09-26 13:41:18'),
(3,	4,	13,	8,	'rejected',	NULL,	'2022-09-27 09:42:44',	'2022-09-27 09:42:44'),
(4,	5,	13,	8,	'rejected',	NULL,	'2022-09-27 09:47:06',	'2022-09-27 09:47:06')
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `submission_id` = VALUES(`submission_id`), `user_id` = VALUES(`user_id`), `department_id` = VALUES(`department_id`), `status` = VALUES(`status`), `note` = VALUES(`note`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
(1,	1,	1,	NULL,	'2022-09-25 12:52:39',	'2022-09-25 12:52:39',	NULL),
(2,	1,	8,	NULL,	'2022-09-27 08:14:20',	'2022-09-27 08:14:20',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `department_id` = VALUES(`department_id`), `description` = VALUES(`description`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
(1,	1,	1,	1,	'2022-09-25 12:52:39',	'2022-09-25 12:52:39',	NULL),
(2,	1,	8,	2,	'2022-09-25 12:52:39',	'2022-09-25 12:52:39',	NULL),
(3,	1,	11,	3,	'2022-09-25 12:52:39',	'2022-09-25 12:52:39',	NULL),
(4,	2,	8,	1,	'2022-09-27 08:14:20',	'2022-09-27 08:14:20',	NULL),
(5,	2,	11,	2,	'2022-09-27 08:14:20',	'2022-09-27 08:14:20',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `approval_rule_id` = VALUES(`approval_rule_id`), `department_id` = VALUES(`department_id`), `approval_order` = VALUES(`approval_order`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
INSERT INTO `companies` (`id`, `company_name`, `company_description`, `monthly_balance`, `phone`, `tc`, `created_at`, `updated_at`) VALUES
(1,	'Koperasi AMM Handil',	NULL,	150000000.00,	'+6281352786183',	'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur convallis ac lorem id laoreet. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae; Nam augue elit, tristique rhoncus fringilla non, luctus a risus. Vivamus turpis orci, ultrices id purus eu, condimentum rhoncus erat. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Quisque semper nibh eros, ut convallis augue dapibus sit amet. Ut efficitur erat sed felis sollicitudin luctus. Nullam consequat tellus nibh, eget efficitur felis efficitur quis. Aliquam augue dui, rutrum congue ex tempor, ullamcorper vulputate turpis.\r\n\r\nPellentesque sit amet mi tortor. Nulla luctus sagittis orci ut fermentum. Ut condimentum, sem eu tempus commodo, enim lorem blandit arcu, at semper est ipsum aliquet lacus. Curabitur mattis nisl in felis consectetur consectetur. Vestibulum feugiat eros in ligula gravida, nec accumsan justo fringilla. In imperdiet nisi ac quam tincidunt commodo. Donec commodo nulla non libero cursus hendrerit. Sed id mi enim. Cras euismod tristique purus, vitae tristique lacus posuere a. Ut pulvinar mollis urna, at faucibus mauris finibus ac.\r\n\r\nMorbi vestibulum at neque non consequat. Phasellus euismod sollicitudin tortor, ut tristique enim gravida nec. Quisque convallis quam nec urna rutrum malesuada. Etiam nisl dolor, malesuada eu erat nec, sodales vehicula est. Mauris eget scelerisque nisi. Nam pretium tristique leo, ac malesuada urna ornare in. Integer tempor neque sit amet congue euismod. In nisi ex, accumsan sed dignissim sed, tempor non est.\r\n\r\nNam blandit tristique orci sed gravida. Quisque non sem sodales tellus gravida auctor. Pellentesque in eleifend odio. Sed id scelerisque ante. Vivamus at gravida ex, et malesuada erat. Nunc tristique ante sem, quis scelerisque neque sodales et. Etiam eros quam, ultricies at euismod nec, iaculis in nisl. Phasellus consectetur, magna non condimentum congue, nulla nisl placerat diam, quis ultricies leo nisi in erat. Aenean a fringilla nulla, quis ornare nulla. Vivamus feugiat sodales mi vel laoreet. Etiam urna tellus, lobortis a neque sed, dictum pulvinar tortor. Donec ac nisi eget purus iaculis lobortis. Nulla condimentum tellus vitae augue viverra, sit amet aliquet leo faucibus. Quisque id iaculis magna, eget molestie est. Sed pretium urna erat, nec viverra metus molestie nec. Aliquam pharetra, nulla nec varius sagittis, ante lacus luctus dui, sit amet commodo libero libero eu turpis.\r\n\r\nFusce lobortis sodales scelerisque. Phasellus ligula lectus, ullamcorper vel lectus et, lacinia sodales mauris. Fusce tincidunt sapien in orci sodales pharetra. Nulla a nulla arcu. Suspendisse id ultrices eros. Quisque odio lorem, cursus vel erat ut, malesuada accumsan arcu. Duis lobortis risus ante. Donec ultrices mauris nisi, et imperdiet ante vehicula eu. Integer hendrerit vulputate dui ac volutpat. Suspendisse potenti. Maecenas sed orci tincidunt, mattis erat non, viverra leo. Duis volutpat egestas metus vel vestibulum. Aenean dignissim tempor libero, eget venenatis leo scelerisque sit amet. Curabitur tristique finibus lectus, ut viverra massa imperdiet pharetra.',	'2022-08-16 05:17:47',	'2022-09-29 01:40:52'),
(5,	'Koperasi AMM Site Balikpapan',	NULL,	500000000.00,	'+62 81254982661',	NULL,	'2022-09-25 13:00:08',	'2022-09-25 13:00:08')
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_name` = VALUES(`company_name`), `company_description` = VALUES(`company_description`), `monthly_balance` = VALUES(`monthly_balance`), `phone` = VALUES(`phone`), `tc` = VALUES(`tc`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
INSERT INTO `credits` (`id`, `submission_id`, `month`, `amount`, `paid`, `billed_at`, `created_at`, `updated_at`) VALUES
(7,	6,	1,	450000.00,	0,	'2022-10-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25'),
(8,	6,	2,	450000.00,	0,	'2022-11-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25'),
(9,	6,	3,	450000.00,	0,	'2022-12-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25'),
(10,	6,	4,	450000.00,	0,	'2023-01-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25'),
(11,	6,	5,	450000.00,	0,	'2023-02-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25'),
(12,	6,	6,	450000.00,	0,	'2023-03-01',	'2022-09-12 17:55:25',	'2022-09-12 17:55:25')
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `submission_id` = VALUES(`submission_id`), `month` = VALUES(`month`), `amount` = VALUES(`amount`), `paid` = VALUES(`paid`), `billed_at` = VALUES(`billed_at`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
(36,	203,	1,	1.00,	1.00,	'2022-09-04 07:23:01',	'2022-09-04 07:23:01',	NULL),
(37,	203,	3,	1.00,	1.00,	'2022-09-04 07:23:01',	'2022-09-04 07:23:01',	NULL),
(38,	203,	6,	1.00,	1.00,	'2022-09-04 07:23:01',	'2022-09-04 07:23:01',	NULL),
(39,	203,	9,	1.00,	1.00,	'2022-09-04 07:23:01',	'2022-09-04 07:23:01',	NULL),
(40,	203,	12,	1.00,	1.00,	'2022-09-04 07:23:01',	'2022-09-04 07:23:01',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `product_id` = VALUES(`product_id`), `count` = VALUES(`count`), `price` = VALUES(`price`), `credit` = VALUES(`credit`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
(11,	1,	'Koperasi',	'2022-09-10 09:58:01',	'2022-09-10 09:58:01',	NULL),
(12,	1,	'PPA ACADEMY',	'2022-09-27 07:52:57',	'2022-09-27 07:52:57',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `department_name` = VALUES(`department_name`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `position_name` = VALUES(`position_name`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
(6,	11,	1,	1,	1,	'User 1',	'NIK111111',	'007',	'KTP111111',	'2022-09-25 12:53:33',	'2022-09-25 12:53:33',	NULL),
(7,	12,	1,	1,	1,	'IT PIC',	'NIK222222',	'008',	'KTP222222',	'2022-09-25 12:54:46',	'2022-09-25 12:54:46',	NULL),
(8,	13,	8,	1,	1,	'HCGA PIC',	'NIK333333',	'009',	'KTP333333',	'2022-09-25 12:55:49',	'2022-09-25 12:55:49',	NULL),
(9,	14,	11,	1,	1,	'Koperasi PIC',	'NIK444444',	'010',	'KTP444444',	'2022-09-25 12:56:22',	'2022-09-25 12:56:22',	NULL),
(10,	16,	12,	1,	1,	'PPA ACADEMY PIC',	'NIK666666',	'012',	'KTP666666',	'2022-09-27 07:54:07',	'2022-09-27 07:54:07',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `user_id` = VALUES(`user_id`), `department_id` = VALUES(`department_id`), `position_id` = VALUES(`position_id`), `grade_id` = VALUES(`grade_id`), `employee_name` = VALUES(`employee_name`), `nik` = VALUES(`nik`), `phone` = VALUES(`phone`), `ktp` = VALUES(`ktp`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
(2,	4,	'A1',	10000000.00,	'2022-09-04 07:21:50',	'2022-09-04 07:21:50',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `grade_name` = VALUES(`grade_name`), `max_credit` = VALUES(`max_credit`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `user_id` = VALUES(`user_id`), `client_id` = VALUES(`client_id`), `name` = VALUES(`name`), `scopes` = VALUES(`scopes`), `revoked` = VALUES(`revoked`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `expires_at` = VALUES(`expires_at`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `user_id` = VALUES(`user_id`), `name` = VALUES(`name`), `secret` = VALUES(`secret`), `provider` = VALUES(`provider`), `redirect` = VALUES(`redirect`), `personal_access_client` = VALUES(`personal_access_client`), `password_client` = VALUES(`password_client`), `revoked` = VALUES(`revoked`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `client_id` = VALUES(`client_id`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
(16,	1,	'Aida Rutherford',	'Adeline',	'Quasi explicabo mollitia assumenda eos incidunt. Quas fugiat eligendi et est. Eos optio occaecati pariatur eveniet sint. Delectus et ut necessitatibus reprehenderit.',	'https://via.placeholder.com/200',	1,	'2022-08-24 17:05:13',	'2022-09-09 01:51:06',	'2022-09-09 01:51:06'),
...
(131,	1,	'Agustina Thiel',	'Antonietta',	'Praesentium dignissimos doloribus eum quae voluptate autem. Debitis nobis dolor iste. Et sunt deleniti id non qui.',	'images/O64KHrLxWEQ0uGsqmc9FC8JE4COh0hsgeHO5KnG9.jpg',	1,	'2022-08-24 17:05:14',	'2022-09-09 01:50:51',	'2022-09-09 01:50:51'),
...
(161,	1,	'Oppo A16 4/64 GB',	'OPPO',	'Id sapiente error molestias velit ut.\r\nAnimi blanditiis soluta consectetur\r\namet atque tenetur voluptatibus.',	'images/ALIzTQhpxcKXyS5WNtXurbcMEclp6W6p8IjRU5LZ.jpg',	1,	'2022-08-24 17:05:14',	'2022-09-06 05:37:51',	NULL),
...
(193,	1,	'Vivo V17 Plus',	'VIVO',	'Magnam eius distinctio harum cum. Omnis quo et hic aspernatur debitis voluptatibus. Tenetur id deserunt inventore eum maxime magni. Cum facilis enim occaecati facere ut pariatur quia.',	'images/85qcFUiqJmrSrz3ztdDNdEEtGs0BX4U8PbrXWpJm.jpg',	1,	'2022-08-24 17:05:14',	'2022-09-13 03:53:54',	NULL),
...
(203,	4,	'Samsung A300',	'Samsung',	NULL,	'images/kDtQuolBWEEMKhrLopWHl79yq4ML2lmdpfD8JLdh.jpg',	1,	'2022-09-04 07:22:31',	'2022-09-04 07:22:31',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `product_name` = VALUES(`product_name`), `product_brand` = VALUES(`product_brand`), `product_description` = VALUES(`product_description`), `product_image` = VALUES(`product_image`), `active` = VALUES(`active`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
...
CREATE TABLE `submissions` (
  ...
  `payment_status` enum('unpaid','progress','paid','canceled') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'unpaid',
  `phone` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `foto_ktp` text COLLATE utf8mb4_unicode_ci NOT NULL,
  `foto_permit` text COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `submissions` (`id`, `user_id`, `product_id`, `credit_scheme_id`, `product_note`, `submission_status`, `payment_status`, `phone`, `foto_ktp`, `foto_permit`, `status_note`, `created_at`, `updated_at`) VALUES
(1,	11,	161,	22,	NULL,	'rejected',	'canceled',	'007',	'attachment/XFZYbTj5lV4q6MPaM1EhCjHgelYNlhRBrChSg8kt.jpg',	'attachment/GhI4BfkOl3Gr58bZ60EDTeLNJq9ui03MENG8atum.jpg',	'Kurang Mantab',	'2022-09-25 12:56:58',	'2022-09-25 12:58:15'),
(2,	11,	161,	22,	NULL,	'rejected',	'canceled',	'081254982664',	'attachment/JHOhHgyzIS2dwsUjFhyS3nYlNwEdtfcLwpK2eWno.jpg',	'attachment/0tHF8l9AP1YiyU0DRSlfJcl8ABwXmn6JLxEUGC3m.jpg',	NULL,	'2022-09-26 13:40:28',	'2022-09-26 13:41:18'),
(3,	12,	161,	23,	NULL,	'pending',	'unpaid',	'081254982664',	'attachment/yjfBQCQNawjqkfh0dPqtPjw5H60tWKtHf0xbIBOR.jpg',	'attachment/7e27RY6wJbGW1Z2m2BuClvhQ9w8ET4jKjm7MBSr0.jpg',	NULL,	'2022-09-26 13:42:10',	'2022-09-26 13:42:10'),
(4,	13,	193,	19,	NULL,	'rejected',	'canceled',	'081254982664',	'attachment/kaJeG0qoZWd4MAJsv7xA4DG4VmhzcWCDm0KjLFZV.jpg',	'attachment/5GlF9oE1sWnSPsfxDUUTTkVr0LzHl54xXkQsuD50.jpg',	NULL,	'2022-09-27 08:13:15',	'2022-09-27 09:42:44'),
(5,	11,	193,	20,	NULL,	'rejected',	'canceled',	'081254982664',	'attachment/NCpWu5BE4WQRiQa0zCqEdlNSaBG1sTtUPPlFgN06.jpg',	'attachment/WP0pwW9zuzI62tlH9nEE0lUKTh1vEUErjUeRCykl.jpg',	NULL,	'2022-09-27 09:46:24',	'2022-09-27 09:47:06'),
(6,	11,	193,	18,	NULL,	'pending',	'unpaid',	'081254982664',	'attachment/kFTSDLkdi5ZbfRICdVK9PJGSCupOwmZPJzOQnsRl.jpg',	'attachment/THrETSCa5iBforrOAsFK9JFGYvEbu49F32Q0zS9e.jpg',	NULL,	'2022-09-27 09:50:20',	'2022-09-27 09:50:20')
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `user_id` = VALUES(`user_id`), `product_id` = VALUES(`product_id`), `credit_scheme_id` = VALUES(`credit_scheme_id`), `product_note` = VALUES(`product_note`), `submission_status` = VALUES(`submission_status`), `payment_status` = VALUES(`payment_status`), `phone` = VALUES(`phone`), `foto_ktp` = VALUES(`foto_ktp`), `foto_permit` = VALUES(`foto_permit`), `status_note` = VALUES(`status_note`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`);
...
(1,	1,	'admin',	'julian.aryo1989@gmail.com',	'$2y$10$n9iSs/xB0xoyvVllmDAYgeaPeXLJ8z5MRTpMROLZWbh6cJqsv4UwC',	'28ec3114-cd21-4838-b741-c44d0bfcf990',	1,	NULL,	'2022-08-16 05:17:47',	'2022-09-30 01:06:38',	NULL),
(11,	1,	'user',	'user1@test.com',	'$2y$10$.9L4Uw.P8xZn6zdecbO52OlUKKfpzeiNUxxz30bOaXDFGJBsLEjt.',	NULL,	1,	NULL,	'2022-09-25 12:53:33',	'2022-09-29 01:42:16',	NULL),
(12,	1,	'pic',	'user2@test.com',	'$2y$10$xf7GmAoK6Xaqdc7bnNJVb.dZ8Q3w4XuaJuRVm8nyPilTbAlOP49la',	NULL,	1,	NULL,	'2022-09-25 12:54:46',	'2022-09-30 01:06:32',	NULL),
(13,	1,	'pic',	'user3@test.com',	'$2y$10$dn02z1xRI1o06NlzcpQuZ.TBYhloKQcSnybiIWPyEGI4HJW1eRpo.',	'0e93bda7-71e4-440f-8698-1693eacf53b0',	1,	NULL,	'2022-09-25 12:55:49',	'2022-09-29 01:42:33',	NULL),
(14,	1,	'pic',	'user4@test.com',	'$2y$10$frKDBKExOOrHuadpqBKcmOg0oZjufc/wqfiW5MXLvg0oJ7OOEDcBS',	NULL,	1,	NULL,	'2022-09-25 12:56:22',	'2022-09-25 12:56:22',	NULL),
(15,	5,	'admin',	'my.meww@gmail.com',	'$2y$10$hKe8Tw7m4xkzKYLS5RW3me/.fAkhgEP3mrvyRS3UWAiqNR2iBJbxq',	NULL,	1,	NULL,	'2022-09-25 13:00:08',	'2022-09-25 13:01:02',	NULL),
(16,	1,	'pic',	'user6@test.com',	'$2y$10$Qi5uvDuC2el4A4kHpgxjX.x16CejZPhXJow5E26BBBOD6hTeVRRce',	NULL,	1,	NULL,	'2022-09-27 07:54:07',	'2022-09-27 07:54:07',	NULL)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `company_id` = VALUES(`company_id`), `role` = VALUES(`role`), `email` = VALUES(`email`), `password` = VALUES(`password`), `api` = VALUES(`api`), `active` = VALUES(`active`), `remember_token` = VALUES(`remember_token`), `created_at` = VALUES(`created_at`), `updated_at` = VALUES(`updated_at`), `deleted_at` = VALUES(`deleted_at`);
